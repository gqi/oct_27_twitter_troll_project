...
Now we load the tweets commenting on each of the top 100 accounts. Due to API limits, we only managed to download the tweets for the top 74, and the actual retrieved tweets may be fewer than the requested ones.
```{r load_train_tweets}
top100tweets = list()
...
## Step 1: word frequency calculation and screening
...
```
For each tweet, record the number of words that appear in each list.
```{r}
...
```
First step screening by word counts
```{r}
indices = which(rowSums(negwordsNum.pitt.jolie)>0)
data.clean.pitt.logit = data.clean.pitt[indices,]
```

Predict the troll tweets using the logistic model fitted using the training set. The cutoff for predicted probability is 1 - (proportion of troll tweeets in training set). We want to keep the proportion of detected trolls the same as that of the training data. Print the 
```{r}
